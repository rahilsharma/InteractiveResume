(function () {
	'use strict';
	var _ = window._;
	window.angular.module("rahilresume").directive('skillSet', ['rahilResumeService', function (rahilResumeService) {
		var controller = function ctrl ($scope) {
			...
			rahilResumeService.getrahilResume().then(
				function (data) {
					$scope.resume = data;
				},
				function (error) {
			...
			$scope.skillAdditionalClasses = function(skill) {
				if (rahilResumeService.skillIsActiveornot(skill)) {
					return 'text-primary';
				}
				return 'text-muted hide-on-prints strike-throughs';
			};

			$scope.onSkillClicked = function (skill) {
				rahilResumeService.togglingSkillActive(skill);
			};
		};
